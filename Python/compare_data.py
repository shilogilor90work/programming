...
def getStoreName(store_id):
    ...
    try:
        cnx = mc.connect(user='', password='', host='reader-db.wiser.com', database='wp_data_prod')
...
#    old_df.to_csv('old_df.csv', sep='\t', encoding='utf-8')
#    old_df.to_csv('new_df.csv', sep='\t', encoding='utf-8')
    ...
    for sku in old_store_skus:
        ...
        if len(diff) > 0:        # there are competitors that are in the old store BUT not on the new store.
            ...
            firstRowDiff = True
            for comp in diff:
                diffdata = old_df.loc[(old_df['sku'] == sku) & (old_df['competitor_name'] == str(comp)[2:-1])]
                mainOutputDF = mainOutputDF.append({
                                "Product_Name": str(diffdata.product_name.values[0]) if firstRowDiff else None,
                                ...
                                "Sellers": (str(comp)[2:-1]).replace('*','').encode('utf-8'),
